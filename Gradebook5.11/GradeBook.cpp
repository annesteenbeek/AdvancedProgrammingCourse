...
void GradeBook::inputGrades() 
{
   ...
   cout << "Enter the letter grades." << endl
      << "Enter the Q character to end input." << endl;
...
{
   int GPA = (4 * aCount + 3 * bCount + 2 * cCount + dCount) / (aCount+bCount+cCount+dCount);
...
void GradeBook::sortPointerArrayByAverage(Student *students[], int size){
   // Simple implementation of Bubble sort.
   for (int i = 1; i < size; i++) {
      float avg = students[i-1] -> getAverage();
      float avgN = students[i] -> getAverage();
      // compare the values, if larger, place in front ([0] is front)
      if (avg < avgN){
